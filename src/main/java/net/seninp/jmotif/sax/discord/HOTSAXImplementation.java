...
public class HOTSAXImplementation {
    ...
    public static DiscordRecords series2Discords(double[] series, int discordsNumToReport,
        ...
        LOGGER.info("discretized in {}, words: {}, indexes: {}", SAXProcessor.timeToString(start.getTime(), saxEnd.getTime()), sax.getRecords().size(), sax.getIndexes().size());
        ...
        LOGGER.debug("Magic array filled in : {}", SAXProcessor.timeToString(saxEnd.getTime(), hashEnd.getTime()));
        ...
        LOGGER.info("{} discords found in {}", discords.getSize(), SAXProcessor.timeToString(start.getTime(), end.getTime()));
    ...
    private static DiscordRecords getDiscordsWithMagic(double[] series, SAXRecords sax,
        ...
        while (discords.getSize() < discordCollectionSize) {
            ...
            LOGGER.trace("currently known discords: {} out of {}", discords.getSize(), discordCollectionSize);
            ...
            if (bestDiscord.getNNDistance() == 0.0D || bestDiscord.getPosition() == -1) {
                LOGGER.trace("breaking the outer search loop, discords found: {} last seen discord: {}" + discords.getSize(), bestDiscord);
                break;
            }
    ...
    private static DiscordRecord findBestDiscordWithMagic(double[] series, int windowSize,
        ...
        for (MagicArrayEntry currentEntry : allWords) {
            ...
            for (int currentPos : occurrences) {
                ...
                LOGGER.trace("conducting search for {} at {}, iteration {}", currentWord, currentPos, iterationCounter);
                ...
                for (Integer nextOccurrence : occurrences) {
                    ...
                    if (dist < nearestNeighborDist) {
                        ...
                        LOGGER.trace(" ** current NN at {}, distance: {}, pos {}", nextOccurrence, nearestNeighborDist, currentPos);
                    }
                    if (dist < bestSoFarDistance) {
                        LOGGER.trace(" ** abandoning the occurrences loop, distance {} is less than best so far {}", dist, bestSoFarDistance);
                ...
                if (doRandomSearch) {
                    ...
                    while (cIndex >= 0) {
                        ...
                        if (dist < bestSoFarDistance) {
                            nearestNeighborDist = dist;
                            LOGGER.trace(" ** abandoning random visits loop, seen distance {} at iteration {}", nearestNeighborDist, visitCounter);
                ...
                LOGGER.trace(" . . iterated {} times, best distance:  {} for a string {} at {}", iterationCounter, bestSoFarDistance, bestSoFarWord, bestSoFarPosition);
    ...
    public static DiscordRecords series2DiscordsDeprecated(double[] series, int discordsNumToReport,
        ...
        Date hashEnd = new Date();
        LOGGER.debug("Hash filled in : {}", SAXProcessor.timeToString(saxEnd.getTime(), hashEnd.getTime()));
    ...
    private static DiscordRecords getDiscordsWithHash(double[] series, int windowSize,
        ...
        while (discords.getSize() < discordCollectionSize) {
            ...
            // if the discord is null we getting out of the search
            if (bestDiscord.getNNDistance() == 0.0D || bestDiscord.getPosition() == -1) {
                LOGGER.trace("breaking the outer search loop, discords found: {} last seen discord: {}", discords.getSize(), bestDiscord.toString());
                break;
            }
    ...
    private static DiscordRecord findBestDiscordWithHash(double[] series, int windowSize,
        ...
        while (!frequencies.isEmpty()) {
            ...
            LOGGER.trace("conducting search for {} at {}, iteration {}, to go: {}", currentWord, currentPos, iterationCounter, frequencies.size());
            ...
            for (Integer nextOccurrence : currentWordOccurrences) {
                ...
                if (dist < bestSoFarDistance) {
                    LOGGER.trace(" ** abandoning random visits loop, seen distance {} at iteration {}", dist, bestSoFarDistance);
            ...
            if (doRandomSearch) {
                ...
                while (-1 != (randomPos = randomRegistry.getNextRandomUnvisitedPosition())) {
                    ...
                    // keep track
                    if (dist < nearestNeighborDist) {
                        LOGGER.trace(" ** current NN at {}, distance: {}, pos {}" + randomPos, dist, currentPos);
                        nearestNeighborDist = dist;
                    }
